...
public class CrudPessoaController implements Serializable {
	...
	public List<Pessoa> getFilteredPessoas() {
	    if (nameFilter == null || nameFilter.isEmpty()) {
	        filteredPessoas = listaPessoa;
	    } else {
	        filteredPessoas = new ArrayList<>();
	        for (Pessoa pessoa : listaPessoa) {
	            if (pessoa.getNome().toLowerCase().contains(nameFilter.toLowerCase())) {
	                filteredPessoas.add(pessoa);
	            }
	        }
	    }
	...
	public void salvarNovo() {
		if (this.pessoa.getGenero().equalsIgnoreCase("F") || this.pessoa.getGenero().equalsIgnoreCase("M") || this.pessoa.getGenero().equalsIgnoreCase("N"))
		{
